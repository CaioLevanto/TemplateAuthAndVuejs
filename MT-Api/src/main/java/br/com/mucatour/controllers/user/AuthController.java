...
import br.com.mucatour.payload.request.user.TokenVerifyRequest;
...
public class AuthController {
    ...
    public Boolean verifyValidToken(@Valid @RequestBody TokenVerifyRequest token) {
        return jwtUtils.isTokenValid(token.getToken(), userDetailsServiceImpl.loadUserByUsername(token.getEmail()));
    }
    ...
    public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginRequest loginRequest) {
        ...
        User hasUser = userRepository.findById(userDetails.getId())
                            .orElseThrow(() -> new UsernameNotFoundException("Não foi possível localizar usuário"));
    ...
    public ResponseEntity<?> refToken(@Valid @RequestBody TokenRefreshRequest request) {
        ...
        if (rToken.isPresent()) {
            long hasToken = refreshTokenService.verifyExpiration(rToken.get()).getUserId();

            String email = userRepository.findById(hasToken)
                    .map(User::getEmail)
                    .orElseThrow(() -> new UsernameNotFoundException("Não foi possível localizar usuário"));

            return ResponseEntity.ok(new TokenRefreshResponse(
                    jwtUtils.generateTokenFromUsername(email), requestRefreshToken));
        } else {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) throws AuthException {
        ...
        signUpRequest.setPassword(encoder.encode(signUpRequest.getPassword())); //Encoder Password

        User user = User.buildToUser(signUpRequest);
        Personal person = personalRepository.save(Personal
